...
# Link Band SDK Installer for macOS (Apple Silicon)
...
echo -e "${BLUE}         (Apple Silicon / ARM64)${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
...
ARCH=$(uname -m)
if [[ "$ARCH" != "arm64" ]]; then
    echo -e "${RED}❌ Error: This installer is for Apple Silicon Macs (ARM64)${NC}"
    echo -e "${YELLOW}   Please download the Intel version if you have an Intel Mac${NC}"
    echo ""
...
SERVER_FILE="$SCRIPT_DIR/linkband-server-macos-arm64"
...
if [[ ! -f "$SERVER_FILE" ]]; then
    echo -e "${RED}❌ Error: linkband-server-macos-arm64 not found${NC}"
    echo -e "${YELLOW}   Please ensure all files are in the same folder${NC}"
    echo ""
    read -p "Press any key to exit..."
    exit 1
