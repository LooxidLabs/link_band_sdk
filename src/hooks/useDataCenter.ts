...
import type { FileInfo, ExportHistory, SearchParams, ExportParams } from '../types/data-center'; // Import necessary types
...
export const useDataCenter = () => {
  ...
  const handleExport = useCallback(async (params: ExportParams) => { // Typed params
    ...
    try {
      ...
      store.setExportHistory(prev => [result, ...(prev || [])]); // Ensure prev is an array
